...
const SECTION_COMMENTS: Record<string, Record<string, string[]>> = {
  saju: {
    ...
    myo: ["냥~ 이게 네 사주 네 기둥이야! ✨", "위에 있는 글자가 천간이고 아래가 지지래~ 합치면 8글자니까 팔자! 팔자가 여기서 나온 거다냥~", "年은 어릴 때, 月은 청춘, 日은 지금, 時는 나중! 시간 순서대로 읽는 거야~", "네 일간은 壬水! 물처럼 어디든 스며드는 타입이라구 냥냥 💧"],
  },
  ohaeng: {
    ...
    myo: ["오행 차트다냥! 🎨", "木=🌳성장, 火=🔥열정, 土=🏔안정, 金=⚔️결단, 水=💧지혜! 이 다섯 가지가 네 안에 섞여있는 거야~", "木이 30%로 젤 높다냥! 새싹처럼 뭐든 시작하는 거 좋아하는 타입이지? 근데 金이 10%밖에 없어서 끝맺음이 좀... 냐하하 😹", "용신이 金이니까 반짝반짝한 거 가까이 두라냥! 금색 팔찌 같은 거 어때? ✨"],
  },
  daeun: {
    ...
    myo: ["대운 타임라인이다냥! 🎢", "10년마다 운이 확 바뀌는 건데, 롤러코스터 타는 거랑 비슷해! 올라갈 때도 있고 내려갈 때도 있구~", "지금은 甲子 대운! 도약기라구!! 냥이가 보기엔 지금이 네 인생에서 제일 신나는 구간이야~ 🎉", "이때 뭔가 시작하면 대박 날 확률 높다냥! 뭐 망해도 냥이 탓은 하지 마라냥 😹"],
  },
  yearly: {
    ...
    myo: ["2026년 운세 오픈~! 🎊", "丙午년이라 불불불! 🔥🔥🔥 근데 네 壬水랑 만나면 오히려 찰떡궁합이야! 물이 불을 다스리니까~", "연애운 ⬆️ 재물운 ⬆️ 직업운 ⬆️ 거의 다 올라간다냥! 특히 봄에 좋은 일 생길 거야~", "근데 7~8월에 좀 지칠 수 있어... 그때는 냥이처럼 낮잠 자면서 쉬라냥 😴"],
  },
  personality: {
    ...
    myo: ["성격 분석 타임이다냥~! 😼", "壬水 일간이면 기본적으로 똑똑이! 머리 회전이 빠르고 눈치도 좋아~ 냥이가 인정한다냥!", "근데 午火가 붙어있어서 가만히 있는 걸 못 참지? 새로운 거 안 하면 근질근질한 타입이야 ㅋㅋㅋ", "약점은~ 좀 끈기가 부족할 수 있어! 근데 그것도 매력이라구 냥냥 😽"],
  },
  sipsin: {
    ...
    myo: ["십신 관계도다냥! 이건 좀 복잡하지만 냥이가 쉽게 설명해줄게~ 🤓", "십신은 네 사주 속 글자들이 나한테 어떤 의미인지 보는 거야! 친구인지, 도와주는 건지, 시험하는 건지~", "편인이 제일 강하다냥! 편인은 '천재적 재능' 같은 거야! 남들이 못 보는 걸 보는 눈이 있다구~ 👀", "근데 너무 혼자 파고들면 외로워질 수 있으니까 가끔은 나랑 놀아라냥~ 😹"],
  },
  sinsal: {
    ...
    myo: ["특수 능력치 언락이다냥! 🎮✨", "신살은 사주에 숨겨진 히든 스킬이야! 좋은 것도 있고 조심할 것도 있구~", "천을귀인 있다냥!! 이건 SSR급 신살이야! 위기 때 구원자가 나타나는 능력! 부럽다냥~ 🌟", "도화살도 있어?! 매력 MAX 버프! 사람을 홀리는 능력이라구~ 냥이도 홀렸다냥 😻"],
  },
  lucky: {
    ...
    myo: ["행운 부스터 아이템이다냥~! 🚀", "金 기운 채우려면 반짝반짝한 거! 금색 팔찌, 은색 귀걸이, 뭐든 좋아~ 냥이 방울도 금색이다냥 🔔", "방향은 서쪽! 숫자는 4, 9! 로또 살 때 참고하라냥~ (당첨 안 돼도 냥이 탓 아님 😹)", "오후 3~5시가 행운 타임이야! 이때 고양이(=냥이)를 쓰다듬으면 운이 더 올라간다냥~ 🐱✨"],
  },
};
